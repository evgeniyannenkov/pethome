        ...
        .then(( pets ) => {
            ...
            countPromise
                .then(( count ) => {
                    ...
                    rest = count > limit && count % limit > 0 ? 1 : 0;

                    lastPage = Math.round(count / limit) + rest || 1;
...
router.post('/', response.ifLoggedOut(), ( req, res, next ) => {
    ...
    if ( !req.user.blocked ) {
        ...
        if ( req.user.verification.is_verified ) {
            ...
            res.json({
                success : false,
                message : "This account is not verified, you can't create new advert."
            });
        }
    } else {
        res.json({
