...
function imagesComponentsInit ( module, constants ) {
    ...
    module.directive("backgroundBlock", [
        function () {
            return {
                template : `<div ng-if="parallax" class="background-block {{classes}}" ng-paralax ratio="0.1" style="background-image: url('{{url}}');" ng-transclude></div>
                            <div ng-if="!parallax" class="background-block {{classes}}" style="background-image: url('{{url}}');" ng-transclude></div>`,
                scope : {
                    url : "@backgroundBlock",
                    classes : "@",
                    parrallax : "="
    ...
    module.component('ngImage', {
        template : `<div class="image" lightbox-trigger="{{image.src}}" style="background-image: url('{{image.src}}')">
                        ...
                        <div ng-if="image.showActions && image.isMain" class="image__button image__button--top fa fa-check" aria-hidden="true"></div>
